...
\node[box, fill=edgeblue!20]   (edge)  at (2.6,0)   {Edge\\Gateway};
\node[box, fill=kafkapurp!15]  (kafka) at (5.2,0)   {Apache\\Kafka};
\node[box, fill=sparkred!15]   (spark) at (7.8,0)   {Spark\\Executors};

%% BOTTOM ROW
\node[box, fill=rlgreen!15]    (meta)  at (2.6,-2.6) {TD3\\Meta-Ctrl};
\node[box, fill=rlgreen!20]    (ppo)   at (5.2,-2.6) {PPO\\Agent};
\node[box, fill=openfaas!15]   (faas)  at (7.8,-2.6) {OpenFaaS\\Function};

%% TOP ROW ARROWS (horizontal, clear of boxes)
\draw[arr] (iot.east)  -- (edge.west);
\draw[arr] (edge.east) -- (kafka.west);
% Kafka -> Spark: arc above boxes
\draw[arr] (kafka.north) -- ++(0,0.6)
    -- node[above, font=\tiny]{stream} ++(2.6,0)
    -- (spark.north);

%% KAFKA -> RL
% Kafka -> Meta: down then left
...
%% EDGE BURST -> PPO: drop below edge, then right
\draw[arr, dashed, edgeblue] (edge.south) -- ++(0,-0.8) -|
    node[below, font=\tiny, near start]{burst signal} (ppo.north);

%% RL -> Spark via OpenFaaS
\draw[arr, rlgreen] (ppo.east) --
    node[above, font=\tiny]{decision} (faas.west);
\draw[arr, openfaas] (faas.north) -- ++(0,0.55) -|
    node[right, font=\tiny, pos=0.3]{scale} (spark.south);

%% FEEDBACK: Spark -> Meta (loop around right + bottom)
\draw[arr, dashed, sparkred]
    (spark.east) -- ++(0.6,0)
    -- ++(0,-3.55)
